...
| `draft_user_template(template, template_type, anchor, template_format)` | `POST /draft-user-template` | **Step 1 of 2** for a user's own PDF format: returns several treatments of an html template, each rendered as presigned **PNG previews** at the size it will occupy in a report (an `add_on` at its in-report column width; a `bespoke` as full pages) plus a `draft_id`. Nothing is stored — the draft lives 24h. Show the user the previews and let them pick (authed, 30/min) |
| `save_user_template(draft_id, variant_id, template, template_type, template_format, anchor)` | `POST /save-user-template` | **Step 2 of 2**: pass `draft_id` + the chosen `variant_id` and the exact reviewed markup is saved; or pass `template` + `template_type` (+ `anchor`) directly for markdown / no-preview saves. **One template per user — upsert**, saving replaces the previous one. Returns the `placeholders` the compile made dynamic (authed, 100/min) |
| `get_user_template()` | `GET /get-user-template` | The caller's saved template (`template_type`, `template_format`, `anchor`, sanitized `template`, timestamps). 404 = none saved; a 503 means the backend table isn't created yet, **not** "nothing saved" |
...
Want reports in your own format? A user can supply their own HTML (or markdown) template — masthead, colours, layout, branding — and every PDF built for them then follows it. A template is a visual choice, so it is approved by **looking**: `draft_user_template` renders several treatments as PNG previews (an `add_on` replaces one section of the standard layout, named by `anchor`; a `bespoke` drives the whole document), the user picks one, and `save_user_template(draft_id, variant_id)` stores exactly the markup they saw — one template per user, upserted. From then on `list_options("pdf_options")` returns a `user_template` block for that user: the agent fills it with real data and posts it under `document_structure["user_template_main"]` on every `build_pdf_*` call, and the backend does the swap (the response carries `template_applied`). `get_user_template` shows what is in force; `delete_user_template` returns the user to the standard layouts.
