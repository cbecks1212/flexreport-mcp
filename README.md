...
| `get_task_status(task_id, max_rows?, offset?)` | `GET /task-status` | Poll an async job to `SUCCESS` and read its `result`; result-set `rows` are paged in 2,000-row windows (`rows_truncated` → re-poll with `offset`) |
